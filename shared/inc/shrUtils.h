...
extern "C" enum LOGMODES 
{
    ...
    DELTAT =     8, // bit to signal "pre-pend standard Delta Time info"
...
//! Example: shrLog(LOGBOTH | DELTAT, shrDeltaT(1), "Function A\n"); 
//! 
//! Automatically opens file and stores handle if needed and not done yet
//! Closes file and nulls handle on request
//! 
//! @param 0 iLogMode: LOGCONSOLE, LOGFILE, LOGBOTH, APPENDMODE, DELTAT, ERRORMSG, CLOSELOG.  
...
//!          First 3 options may be | 'd  with DELTAT to start line with Standard Delta Time message
...
// *********************************************************************
extern "C" int shrLog(int iLogMode, double dValue, const char* cFormatString, ...);
...
inline void __shrEXIT(int argc, const char** argv)
{
    ...
    {        
        shrLog(LOGBOTH | CLOSELOG, 0.0, "\nPress <Enter> to Quit...\n");                  
        getchar();                                                           
    }       
    else 
    {
        shrLog(LOGBOTH | CLOSELOG, 0.0, "%s Exiting...\n", argv[0]); 
    }

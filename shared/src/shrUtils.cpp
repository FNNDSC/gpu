...
// *********************************************************************
int shrLog(int iLogMode = LOGCONSOLE, double dValue = 0.0, const char* cFormatString = "", ...)
{
    ...
    // if the file is closed and the call incudes a "write-to-file", open file for writing
    if ((pFileStream == NULL) && (iLogMode & LOGFILE))
    {
        ...
        if (cLogFilePathAndName == NULL)
        {
            ...
            errno_t err;
            if ((err = fopen_s(&pFileStream, cLogFilePathAndName, cFileMode)) != 0)
            {
            ...
            // open the file in the requested mode
            if ((pFileStream = fopen(cLogFilePathAndName, cFileMode)) == 0)
            {
                // if error on attempt to open, be sure the file is null or close it, then return negative error code
                if (pFileStream)
                {
    ...
    // Handle special Delta Time code
    if (iLogMode & DELTAT)  
    {   
        // print string to console if flagged
        if (iLogMode & LOGCONSOLE) 
        {
            szNumWritten = printf (" DeltaT (s) = \t%.6f\t", dValue);
        }
        // print string to console if flagged
        if (iLogMode & LOGCONSOLE) 
        {
            szNumWritten = fprintf (pFileStream, " DeltaT (s) = \t%.6f\t", dValue);
        }
    }
    ...
    {   
        ...
        if (iLogMode & LOGCONSOLE) 
        {
            szNumWritten = printf ("\n !!! Error # %i at ", (int)dValue);
        }
        // print string to console if flagged
        if (iLogMode & LOGCONSOLE) 
        {
            szNumWritten = fprintf (pFileStream, "\n !!! Error # %i at ", (int)dValue);
        }
    }
    ...
    for (pStr = cFormatString; *pStr; ++pStr)
    {
        ...
        if (*pStr != '%')
        {
            // not "%", so print verbatim to console and/or file if flagged
            if (iLogMode & LOGCONSOLE) 
            {
            ...
            if (iLogMode & LOGFILE)
            {
                szNumWritten  = putc(*pStr, pFileStream);
            }
        } 
        else 
        {
            ...
            switch (*++pStr)
            {
                case 's':   // string with default precision (max length for string)
                {
                    ...
                    if (iLogMode & LOGFILE)
                    {
                        szNumWritten = fprintf (pFileStream, "%s", cArg);
                    }
                    continue;
                }
                ...
                case 'i':   // integer with default precision
                {
                    ...
                    if (iLogMode & LOGFILE)
                    {
                        szNumWritten = fprintf (pFileStream, "%i", iArg);
                    }
                    continue;
                }
                case 'u':   // unsigned integer with default precision
                {
                    ...
                    if (iLogMode & LOGFILE)
                    {
                        szNumWritten = fprintf (pFileStream, "%u", uiArg);
                    }
                    continue;
                }
                ...
                case 'E':   // scientific double/float with default precision
                {
                    ...
                    if (iLogMode & LOGFILE)
                    {
                        szNumWritten = fprintf (pFileStream, "%e", dArg);
                    }
                    continue;
                }
                case 'f':  // float/double with default precision
                {
                    ...
                    if (iLogMode & LOGFILE)
                    {
                        szNumWritten = fprintf (pFileStream, "%f", dArg);
                    }
                ...
                case '.':  // Type and Precision are specified (f, e, E, i, d, u, s)
                {
                    ...
                    switch(cFormat[3])
                    {
                        ...
                        case 'E':
                        {
                            ...
                            if (iLogMode & LOGFILE)
                            {
                                szNumWritten = fprintf (pFileStream, cFormat, dArg);
                            }
                            break;
                        }
                        case 'd':
                        case 'i':
                        {
                            ...
                            if (iLogMode & LOGFILE)
                            {
                                szNumWritten = fprintf (pFileStream, cFormat, iArg);
                            }
                            break;
                        }
                        case 'u':
                        {
                            ...
                            if (iLogMode & LOGFILE)
                            {
                                szNumWritten = fprintf (pFileStream, cFormat, uiArg);
                            }
                            break;
                        }
                        case 's':
                        {
                            ...
                            if (iLogMode & LOGFILE)
                            {
                                szNumWritten = fprintf (pFileStream, cFormat, cArg);
                            }
                            break;
                        }
                    }
    ...
    // end the log(s) with a horizontal line if closing
    ...
    if ((pFileStream) && (iLogMode & CLOSELOG))
    {
        fclose (pFileStream);
        pFileStream = NULL;
    }
...
unsigned char* shrLoadRawFile(const char* filename, size_t size)
{
    ...
    #endif
        {
            shrLog(LOGBOTH, 0.0," Error opening file '%s' !!!\n", filename);
            return 0;
        }
    ...
    shrLog(LOGBOTH, 0.0, " Read '%s', %d bytes\n", filename, read);
